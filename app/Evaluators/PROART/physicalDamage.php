...
class physicalDamage
{
    public static function evaluate(Hazard $risk, float $average)
    {
        ...
        $reports = session('auth:company')->reports->first(fn($_, $risk) => $risk === PROARTHazard::PHYSICAL_DAMAGE->value);
    ...
    private static function needsWeightedAverage(int $initialRating, $reports): bool
    {
        if($initialRating === 1){
            $indicators = [
                ...
                session('auth:company')->accidents() && session('auth:company')->accidents() < 10,
            ];
        }
        ...
        if($initialRating === 2){
            $indicators = [
                ...
                session('auth:company')->accidents() && session('auth:company')->accidents() >= 10 && session('auth:company')->accidents() < 25,
            ];
        }
        ...
        if($initialRating === 3){
            $indicators = [
                ...
                session('auth:company')->accidents() && session('auth:company')->accidents() >= 25 && session('auth:company')->accidents() < 40,
            ];
        }
        ...
        if($initialRating === 4){
            $indicators = [
                ...
                session('auth:company')->accidents() && session('auth:company')->accidents() > 40,
            ];
        }
    ...
    private static function weightedAverage(int $initialRating, $reports): int
    {
        ...
        $metrics = [
            ...
            session('auth:company')->accidents(),
        ];
