...
class frustrationOrDemotivation
{
    public static function evaluate(Hazard $risk, float $average)
    {
        $initialRating = self::initialRating($average);
        
        $reports = session('auth:company')->reports->first(fn($_, $risk) => $risk === PROARTHazard::FRUSTRATION_OR_DEMOTIVATION->value);
    ...
    private static function needsWeightedAverage(int $initialRating, $reports): bool
    {
        ...
        if($initialRating === 3){
            $indicators = [
                $reports && $reports > 10,
                session('auth:company')->absenteeism() && session('auth:company')->absenteeism() > 25,
                session('auth:company')->turnover() && session('auth:company')->turnover() > 25,
            ];
        }
    ...
    private static function weightedAverage(int $initialRating, $reports): int
    {
        ...
        $weightedAverage = ($scoreWeight * $initialRating) +
                  ($metricWeight * ($metrics[0] ? PROARTRiskService::metricToProbabilityScale($metrics[0], 3) : 0)) +
                  ($metricWeight * ($metrics[1] ? PROARTRiskService::metricToProbabilityScale($metrics[1], 3) : 0));
                  ($metricWeight * ($metrics[2] ? PROARTRiskService::metricToProbabilityScale($metrics[2], 3) : 0));
