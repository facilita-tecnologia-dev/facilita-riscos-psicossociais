...
class PROARTService
{
    public static function dashboard()
    {
        ...
        $groupedRisks = $campaign->collection()->hazards()
            ->with([
        ...
        $dashboard = $groupedRisks->mapWithKeys(function($risks, $group) {
            $groupEvaluated = $risks->mapWithKeys(function($risk) {    
                ...
                $evaluated = PROARTRiskService::evaluate($risk, $riskAverage);
    ...
    public static function departments(Hazard $hazard)
    {
        ...
        $departments = $hazard->questions()
            ...
            ->mapWithKeys(function($user, $department) use($hazard) { // Calcula o risco de cada usuário
                $userEvaluated = $user->mapWithKeys(function($answers, $user) use($hazard) {
