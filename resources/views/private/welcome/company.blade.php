                    ...
                    @if($currentStep == 4)
                        <div class="w-full max-w-[450px] flex flex-col items-center p-6 gap-6 bg-gray-100 rounded-md shadow-md">
                            ...
                            @if(session('auth:company')->usesHSE())
                                ...
                                <p class="text-sm text-center w-full">Cadastre os Indicadores Epidemiológicos</p>
                                <x-action variant="secondary" width="full" :href="route('company-absence.index')">Cadastrar Indicadores Epidemiológicos</x-action>
                            @else
                                <h2 class="text-sm md:text-base flex items-center gap-3">
                                    <i class="fa-solid fa-percent text-lg"></i>
                                    Dados de Desempenho
            ...
            @else
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
                    <div @if(!session('auth:company')->latestPsychosocialCampaign() || !session('auth:company')->latestPsychosocialCampaign()->userCollections()->exists()) data-tippy-content="Você ainda não realizou testes de Riscos Psicossociais" @endif>
                        <a href="{{ route('dashboard.psychosocial') }}" class="w-full bg-gray-100 rounded-md shadow-md p-4 flex items-center gap-3 relative left-0 top-0 hover:left-0.5 hover:-top-0.5 transition-all {{ !session('auth:company')->latestPsychosocialCampaign() || !session('auth:company')->latestPsychosocialCampaign()->userCollections()->exists() ? 'pointer-events-none opacity-50' : '' }}">
