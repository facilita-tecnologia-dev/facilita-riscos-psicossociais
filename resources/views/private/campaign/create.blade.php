        ...
        <x-structure.main-content-container>
            ...
            <div class="w-full bg-gray-100 rounded-md shadow-md p-4 md:p-8 space-y-6">
                <x-form action="{{ route('campaign.store') }}" id="form-create-campaign" post class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <x-form.select name="collection_id" label="Formulário de Pesquisa" :options="$collections" value="{{ session('auth:company')->psychosocialCollection()->id }}" />
                    
                    <x-form.input-date type="datetime-local" value="{{ \Carbon\Carbon::now()->addMinutes(15)->format('Y-m-d\TH:i') }}" name="start_date" min="{{ \Carbon\Carbon::now()->format('Y-m-d\TH:i') }}" label="Data de início"/>
