    ...
    <div class="bg-secondary-background border-borders flex h-full w-full flex-col items-center justify-center gap-8 overflow-auto border px-6 py-8 shadow-sm sm:h-fit sm:justify-start sm:rounded-2xl">
        ...
        @if ($step === \App\Livewire\Auth\Login\UserLoginComponent::STEPS['SELECT_COMPANY'])
            <div class="flex flex-col gap-4">
                <p class="font-text text-main-text text-center text-xs font-normal sm:text-left sm:text-sm">Você está vinculado a mais de uma empresa. Por favor, escolha abaixo em qual delas deseja se autenticar.</p>
        ...
        @if ($step === \App\Livewire\Auth\Login\UserLoginComponent::STEPS['TEMP_PASSWORD'])
            <form class="grid w-full grid-cols-1 gap-4" wire:submit.prevent="validateTemporaryPassword">
                <p class="font-text text-main-text text-center text-xs font-normal sm:text-left sm:text-sm">Você ainda não possui uma senha definitiva. Acesse com sua senha provisória para criar uma nova senha.</p>
        ...
        @if ($step === \App\Livewire\Auth\Login\UserLoginComponent::STEPS['NEW_PASSWORD'])
            <form class="grid w-full grid-cols-1 gap-4" wire:submit.prevent="updatePassword">
                <p class="font-text text-main-text text-center text-xs font-normal sm:text-left sm:text-sm">Agora, defina e confirme sua nova senha. Ela só será solicitada quando você tentar se autenticar em uma empresa na qual você atua como gestor.</p>
