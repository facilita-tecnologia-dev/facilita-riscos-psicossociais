    ...
    <div class="bg-secondary-background border-borders flex h-full w-full flex-col items-center justify-center gap-8 overflow-auto border px-6 py-8 shadow-sm sm:h-fit sm:justify-start sm:rounded-2xl">
        ...
        @if ($step === \App\Livewire\Auth\Login\UserLoginComponent::STEPS['TEMP_PASSWORD'])
            <form class="grid w-full grid-cols-1 gap-4" wire:submit.prevent="validateTemporaryPassword">
                ...
                <x-form.input-text wireModel="temp_password" name="temp_password" label="Senha Temporária" placeholder="Digite a senha temporária..." tooltip="Digite a senha temporária..." isRequired isPassword />
        ...
        @if ($step === \App\Livewire\Auth\Login\UserLoginComponent::STEPS['NEW_PASSWORD'])
            <form class="grid w-full grid-cols-1 gap-4" wire:submit.prevent="updatePassword">
                ...
                <x-form.input-text wireModel="new_password" name="new_password" label="Nova Senha" placeholder="Digite a nova senha..." tooltip="Digite a nova senha" isRequired isPassword />
                <x-form.input-text wireModel="new_password_confirmation" name="new_password_confirmation" label="Confirme a nova senha" placeholder="Confirme a nova senha..." tooltip="Confirme a nova senha" isRequired isPassword />
        ...
        @if ($step === \App\Livewire\Auth\Login\UserLoginComponent::STEPS['FINAL_PASSWORD'])
            <form class="grid w-full grid-cols-1 gap-4" wire:submit.prevent="validateFinalPassword">
                <x-form.input-text wireModel="password" name="password" label="Senha" placeholder="Digite sua senha..." tooltip="Digite sua senha" isRequired isPassword />
