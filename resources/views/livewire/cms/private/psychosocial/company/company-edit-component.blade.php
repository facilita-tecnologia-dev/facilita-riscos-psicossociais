<form class="bg-secondary-background border-borders flex flex-col gap-4 rounded-lg border p-4 shadow-sm md:p-6" wire:submit.prevent="submit">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <x-new-components.info-item label="{{ 'Campanha de Riscos Psicossociais ' . now()->year }}" :value="$psychosocialCampaignStatus" truncate />
        <x-new-components.info-item label="Facilita Canal de Denúncias" value="{{ $hasReportChannel ? 'Sim' : 'Não' }}" truncate />
    </div>
