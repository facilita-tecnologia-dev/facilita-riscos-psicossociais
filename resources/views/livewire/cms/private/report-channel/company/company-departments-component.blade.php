    ...
    <div class="space-y-4">
        <header class="flex w-full flex-col-reverse justify-start gap-4 sm:flex-row sm:items-center sm:justify-end">
            ...
            <form wire:submit.prevent="createDepartment" class="flex gap-2 sm:max-w-[500px] sm:flex-1">
                <x-form.input-text wireModel="new_department" name="new_department" placeholder="Digite o setor para adicionar..." isRequired />
    
                <x-actions.button class="!bg-report-channel-primary-solid" fitSize>
        ...
        <div>
            @if (isset($departments) && count($departments) > 0)
                <x-table>
                    <x-table.thead class="grid-cols-1">
                        <x-table.th>
                    ...
                    <x-table.tbody>
                        ...
                        @foreach ($departments as $department)
                            <x-table.tr wire:key="department-{{ $department['id'] }}" class="grid-cols-1" last="{{ $loop->last ? true : false }}" data-tippy-content="{{ $department['reports_count'] }} denúncias, {{ $department['users_count'] }} colaboradores">
                                <x-table.td>
                                    ...
                                    @if (!empty($department['deleted_at']))
                                        <x-circle-pulse color="bg-danger" size="size-2.5" tooltip="Desativado" />
                                    @else
                                        <x-circle-pulse color="bg-success" size="size-2.5" tooltip="Ativado" />
                                    @endif
    
                                    @if ($department['reports_count'] || $department['users_count'])
                                        @if (!empty($department['deleted_at']))
                                            <x-actions.button wire:click="restoreDepartment({{ $department['id'] }})" slim fitSize class="!bg-report-channel-primary-solid !h-8 !w-8 !p-0">
                                        ...
                                        @else
                                            <x-actions.button wire:click="softDeleteDepartment({{ $department['id'] }})" slim fitSize class="!bg-report-channel-primary-solid !h-8 !w-8 !p-0" data-tippy-content="Clique para desativar o setor (não aparecerá para novas denúncias)">
                                        ...
                                        @endif
                                    @else
                                        <x-actions.button wire:click="deleteDepartment({{ $department['id'] }})" slim fitSize class="!bg-danger !h-8 !w-8 !p-0" data-tippy-content="Clique para excluir o setor do sistema">
