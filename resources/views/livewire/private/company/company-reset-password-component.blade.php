<div class="contents" x-data="{ resetPasswordModalOpen: false }" x-on:open-reset-password-modal.window="resetPasswordModalOpen = true" x-on:close-reset-password-modal.window="resetPasswordModalOpen = false">
    <div class="bg-secondary-background border-borders flex flex-col items-center gap-2 rounded-lg border px-6 py-4 shadow-sm sm:flex-row">
        ...
        <x-actions.button wire:click="openResetPasswordModal" fitSize>
    ...
    <div x-show="resetPasswordModalOpen" x-transition.opacity x-cloak class="fixed inset-0 z-30 flex items-center justify-center bg-black/60 px-4">
        <div x-on:click.away="$wire.closeResetPasswordModal()" class="bg-secondary-background border-borders flex flex-col gap-4 w-full max-w-xl rounded-lg border p-6 shadow-sm">
            ...
            <form class="grid w-full grid-cols-1 gap-4" wire:submit.prevent="submit">
                <x-form.input-text wireModel="password" name="password" label="Senha atual" placeholder="Digite a senha atual..." tooltip="Digite a senha atual da empresa." isRequired isPassword />

                <x-form.input-text wireModel="new_password" name="new_password" label="Nova senha" placeholder="Digite a nova senha..." tooltip="Crie uma senha de 8 a 30 caracteres, com pelo menos uma letra maiúscula, uma letra minúscula e um caractere especial para maior segurança" isRequired isPassword />
                <x-form.input-text wireModel="new_password_confirmation" name="new_password_confirmation" label="Confirme sua nova senha" placeholder="Confirme a senha..." tooltip="Confirme a senha que você criou" isRequired isPassword />
