    ...
    @foreach ($this->controlActions as $risk => $gravities)        
        <div class="space-y-4 p-4 md:p-5 lg:p-6 bg-main-background rounded-md shadow-md">
            <div class="w-full rounded-md shadow-md p-4 flex items-center gap-2 bg-gray-500">
                <h2 class="text-base sm:text-lg text-main-background text-center font-semibold flex-1">{{ App\Enums\PROART\PROARTHazard::from($risk)->label() }}</h2>
            </div>
            <div class="space-y-4">
                @foreach ($gravities as $gravity => $types)
                    <div class="space-y-5">
                        ...
                        <div class="w-full rounded-md shadow-md px-4 py-2 flex items-center gap-2
                            {{ $gravity == App\Enums\PROART\PROARTRisk::CRITICAL->value ? 'bg-[#F26C6C75]' : '' }}
                            {{ $gravity == App\Enums\PROART\PROARTRisk::HIGH->value ? 'bg-[#F6B26B75]' : '' }}
                            {{ $gravity == App\Enums\PROART\PROARTRisk::MEDIUM->value ? 'bg-[#DDE26F75]' : '' }}
                            {{ $gravity == App\Enums\PROART\PROARTRisk::LOW->value ? 'bg-[#A8E6CFCC]' : '' }}
                        ">
                            <h2 class="text-sm sm:text-base text-center font-semibold flex-1">
                                {{ App\Enums\PROART\PROARTRisk::from($gravity)->label() }}
                            </h2>
                        </div>
