<div class="contents" x-data="{ videoModalOpen: false }" x-on:open-video-modal.window="videoModalOpen = true" x-on:close-video-modal.window="videoModalOpen = false">
    <x-new-components.structure.page-header icon="{{ $campaign->collection()->type == App\Enums\BaseCollectionType::PSYCHOSOCIAL ? 'brain' : 'cloud' }}" label="Responder Teste" :breadcrumbs="[
        $campaign->name => null,
    ...
    <section id="answer-test" class="flex flex-col lg:grid lg:grid-cols-3 gap-4 items-start">
        <div class="w-full bg-secondary-background border-borders lg:col-span-2 flex flex-col items-start gap-8 rounded-2xl border px-4 py-6 sm:px-6 sm:py-8 shadow-sm">
            <header class="flex w-full items-center justify-between">
                ...
                <div class="flex flex-col gap-2">
                    @php
                        ...
                        $options = ($collection->type->value == 'psychosocial-risks' ?  ($collection->key == 'hse' 
                                ? array_map(fn($option) => ['label' => $option->label(), 'value' => $questions[$current]['inverted'] ? $option->inverted() : $option->value] , App\Enums\HSE\HSEOption::cases())
                                : array_map(fn($option) => ['label' => $option->label(), 'value' => $questions[$current]['inverted'] ? $option->inverted() : $option->value] , App\Enums\PROART\PROARTOption::cases())
                            )
