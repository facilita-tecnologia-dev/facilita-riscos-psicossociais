...
<html lang="pt-BR">
    ...
    <body>
        ...
        @foreach ($dashboard['division-factor-satisfaction-by-group'] as $group => $questions)
            <table style="margin-bottom: 8px;">
                <thead>
                    <tr>
                        <th colspan="6" style="text-align: left;">{{ App\Enums\OC\OCGroup::from($group)->label() }}</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($questions as $question)
                        <tr>
                            <td colspan="6" style="width: 100%; font-size: 10px;">{{ $question['statement'] }}</td>
                        </tr>
                        @foreach ($question['satisfaction']->chunk(3) as $chunk)
                            <tr>
                                @foreach ($chunk as $divisionFactor => $satisfaction)
                                    <td style="width: 90%; font-size: 10px">{{ $divisionFactor }}</td>
                                    <td style="width: 10%; font-size: 10px">{{ $satisfaction }}%</td>
                                @endforeach

                                @php
                                    $columnsUsed = $chunk->count() * 2;
                                    $columnsMissing = 6 - $columnsUsed;
                                @endphp

                                @if ($columnsMissing > 0)
                                    <td colspan="{{ $columnsMissing }}"></td>
                                @endif
                            </tr>
                        @endforeach
                    @endforeach           
