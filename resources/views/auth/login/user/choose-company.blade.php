        ...
        <div class="flex h-full justify-center">
            <div class="w-full max-w-[600px] bg-gray-100 flex justify-center items-center px-4">
                <div class="w-full max-w-[400px] flex flex-col items-center gap-8">
                    ...
                    <div class="w-full flex flex-col gap-2 max-h-64 overflow-auto px-2 py-2">
                        @foreach ($user->companies as $company)
                            <x-form action="{{ route('user.login.login-with-company', ['user' => $user, 'company' => $company]) }}" post>
                                <button type="submit" class="block cursor-pointer rounded-md shadow-md bg-gray-100/50 border border-gray-200 w-full px-2 py-3 text-center relative left-0 top-0 hover:left-0.5 hover:-top-0.5 transition-all">
                                    {{ $company->name }}
                                </button>
                            </x-form>
                        @endforeach 
