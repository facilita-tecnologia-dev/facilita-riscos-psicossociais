<div class="p-4 flex flex-col gap-4 rounded-lg bg-main-background border {{ isset($featured) && $featured ? 'border-report-channel-primary-solid' : 'border-borders' }}">
    ...
    @if(isset($main))
        <div class="flex items-center justify-between py-1 border-b border-borders">
            <span class="text-xs md:text-sm text-report-channel-primary-solid font-semibold text-left">{{ $main['label'] }}</span>
            <span class="text-xs md:text-sm text-report-channel-primary-solid font-semibold text-left">{{ $main['value'] }}</span>
        </div>
    @endif
