...
<div class="flex w-full flex-col-reverse items-start gap-1">
    ...
    <input type="color" name="{{ $name }}" id="{{ $name }}" wire:model.defer="{{ $wireModel }}" placeholder="{{ $placeholder }}" value="{{ $value }}" {{ $attributes->merge(['class' => 'peer bg-secondary-background border-borders text-main-text font-text placeholder:text-secondary-text focus:shadow-primary-solid/50 h-10 md:h-[45px] w-full rounded-sm border p-0.5 text-sm md:text-base font-normal transition focus:shadow-sm focus:outline-none']) }} />
